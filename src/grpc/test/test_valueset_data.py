...
def test_fetch_data_from_json_url_file_ko(valueset_data):
    url = "file:./src/python/tests/data/valuesets_test.json"
    with pytest.raises(ValueError) as e_info:
...
def test_get_vsdata_by_value_current_ok(valueset_data):
    ...
    assert vs[0].accession_id == "mane.plus_clinical" and vs[
        0].value == "plus_clinical", f'Response accession_id: {vs[0].accession_id} not the expected one: mane.plus_clinical or response value: {vs[0].value} not the expected one: plus_clinical'
...
def test_get_vsdata_by_domain_ok(valueset_data):
    ...
    vs_test = [v.accession_id for v in vs if
               v.accession_id in ('mane.select', 'mane.select2', 'mane.plus_clinical') and v.is_current]
